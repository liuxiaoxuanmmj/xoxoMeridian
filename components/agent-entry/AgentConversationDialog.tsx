...
export default function AgentConversationDialog({ conversation, principalId, titleId, reducedMotion, onClose, onIdentityInvalid }: {
  ...
  const { snapshot, messages: conversationMessages, tasks, draft, setDraft, pending, readError, sendError, sending, send, retry, refresh } = conversation;
  const inputId = useId();
  ...
  return (
    <section className={styles.panel} aria-labelledby={titleId}>
      ...
      <div ref={messages} className={styles.messages} role="log" aria-label="与小助手的消息" aria-live="polite" aria-relevant="additions">
        ...
        {pending.map((record) => (
          <article key={record.clientMessageId} className={`${styles.message} ${styles.humanMessage}`}>
            ...
            {record.state === "failed" && <button type="button" disabled={sending} className={styles.textButton} onClick={() => void retry(record)}>重试原消息</button>}
          </article>
        ))}
      </div>
      ...
      <form className={styles.composer} onSubmit={(event) => { event.preventDefault(); if (!composing.current) send(); }}>
        ...
        />
        <button type="submit" disabled={sending || !draft.trim() || snapshot?.agent.enabled === false} className={styles.sendButton}>发送</button>
      </form>
    </section>
  );
}
